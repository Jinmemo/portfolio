...
<link rel="stylesheet" href="./source/style/css/ensun.css/detailtfview.css">
...
<body>
	...
	<!-- 썸네일 -->
		<%-- <div class="mainbanner">
			<img src="${tf.thumbNail}" class="mainbanner">
		</div> --%>
	...
	<div class="container">
		...
		<div class="mainbody">
			...
			<div class="title-area">
				<h1 class="maintitle">${tf.travelName}</h1>
				<h3 class="editDate">수정일 : ${tf.editDate}</h3>
			</div>
			...
			<div class="like_box">
				...
				<span class="count"> ${tf.reCount } </span>
			</div>
			...
			<div class="schedule">
				<!-- 날짜 null값일 때  / 날짜 값 있을 때 -->
				<c:choose>
					<c:when test="${tf.travelSdate eq null }">
						<span> 상시진행 </span>
					</c:when>
					<c:otherwise>
						<span> 날짜 : ${tf.travelSdate} ~ ${tf.travelEdate} </span>
					</c:otherwise>
				</c:choose>
			</div>
			...
			<div class="swiper-container">
				<div class="swiper-wrapper">
					<c:forEach var="image" items="${tf.travelImg }">
			...
			<div class="tf_content">
				<p>${tf.travelContent}</p>
			</div>

			<div class="editDateB"> 수정일 : ${tf.editDate}</div>
			<div class="inquiry">
			...
			<form action="tfsearch.bo" method="get">
				<div class="head-keyword" id="search-area">
					<input type="text" name="keyword" id="keyword" placeholder="듣고 싶은 여행지 입력" value='${keyword }'> 
